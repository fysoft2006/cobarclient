...
public class MysdalSqlMapClientTemplate extends SqlMapClientTemplate implements DisposableBean {
   ...
   */
  protected Map<String, SqlMapClientTemplate>
      CURRENT_THREAD_SQLMAP_CLIENT_TEMPLATES =
      new HashMap<String, SqlMapClientTemplate>();
  /**
   * we need target set of the shard to set the boundary of distributed data access and negotiate some default settings
   * for MysdalSqlMapClientTemplate, say, default pool size to use.
   */
  ...
  public void afterPropertiesSet() {
    ...
    for (Shard shard : shards) {
      CURRENT_THREAD_SQLMAP_CLIENT_TEMPLATES
          .put(shard.getId(), new SqlMapClientTemplate(shard.getDataSource(), getSqlMapClient()));
    }
  }
  ...
  public int delete(final String statementName, final Object parameterObject) throws DataAccessException {
    ...
    if (shards.size() == 1) {
      return CURRENT_THREAD_SQLMAP_CLIENT_TEMPLATES.get(shards.iterator().next().getId())
  ...
  public Object queryForObject(final String statementName, final Object parameterObject, final Object resultObject)
    ...
    if (shards.size() == 1) {
      return CURRENT_THREAD_SQLMAP_CLIENT_TEMPLATES.get(shards.iterator().next().getId())
          .queryForObject(statementName, parameterObject, resultObject);
    } else {
      return execute(shards, new SqlMapClientCallback<Object>() {
        public Object doInSqlMapClient(SqlMapExecutor executor) throws SQLException {
  ...
  public void queryWithRowHandler(final String statementName, final Object parameterObject, final RowHandler rowHandler)
    ...
    if (shards.size() == 1) {
      CURRENT_THREAD_SQLMAP_CLIENT_TEMPLATES.get(shards.iterator().next().getId())
          .queryWithRowHandler(statementName, parameterObject, rowHandler);
    } else {
  ...
  protected <T> List<T> execute(Set<Shard> shards, final SqlMapClientCallback<T> callback) {
    ...
    if (!exceptions.getCauses().isEmpty()) {
      throw new TransientDataAccessResourceException(
          "one or more errors when performing data access operations against multiple shards", exceptions);
    }
   ...
   * 批量插入
   */
  private List<Integer> batchInsert(final String statementName, final Map<Shard, List<?>> entity) {
